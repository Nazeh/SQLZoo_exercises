...
/* 10-Find the routes involving two buses that can go from Craiglockhart to Lochend.
    Show the bus no. and company for the first bus, the name of the stop 
...
SELECT S.num, S.company, stops.name, E.num, E.company
  ...
  JOIN (SELECT a.num, a.company, b.stop
          FROM route a JOIN route b ON a.company=b.company AND a.num=b.num
         WHERE a.stop = (SELECT id FROM stops WHERE name='Lochend'))E
  ON S.stop = E.stop
  JOIN stops ON id = S.stop;
